  ...
  function nodeGo(config) {
    ...
    node.on('input', function (msg) {
      ...
      if ( node.output === '' ) {
        ...
        msg.err = ('Output field is REQUIRED, currently blank, set to payload');
        node.send([null,msg]);
      }
      if ( node.outputType === '' ) {
        node.outputType = 'msg';
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('Output Type field is REQUIRED, currently blank, set to msg');
        node.send([null,msg]);
      }
        ...
        try {
          switch ( node.inputType ) {
            ...
            default:
              inp = new Date();
              setOutput(msg, node.outputType, node.output, '');
              msg.err = ('Unrecognised Input Type, ' + node.inputType + '. Output has been set to NOW.');
              node.send([null,msg]);
          }
        } catch (err) {
          inp = new Date();
          setOutput(msg, node.outputType, node.output, '');
          msg.err = ('Input property, ' + node.inputType + '.' + node.input + ', does NOT exist. Output has been set to NOW.');
          node.send([null,msg]);
        }
      }
      ...
      if ( moment.tz.zone(node.inTz) === null ) {
        ...
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('Moment: Input Timezone Invalid, reset to UTC - see http://momentjs.com/timezone/docs/#/data-loading/');
        node.send([null,msg]);
        node.inTz = 'UTC'
      }
      if ( moment.tz.zone(node.outTz) === null ) {
        // tz invalid, warn and set to UTC
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('Moment: Output Timezone Invalid, reset to UTC - see http://momentjs.com/timezone/docs/#/data-loading/');
        node.send([null,msg]);
        node.outTz = 'UTC'
      }
      ...
      if (!mDT.isValid()) {
        ...
        if (dtm === 'Invalid Date') {
          setOutput(msg, node.outputType, node.output, '');
          msg.err = ('Unrecognized date string format => ' + inp);
          node.send([null,msg]);
        }
        else {
          mDT = moment(dtm);
      ...
      try {
        ...
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('Locale string invalid - check moment.js for valid strings');
        node.send([null,msg]);
      }
      ...
      if ( node.adjAmount !== 0 ) {
        ...
        if ( isMeasureValid(node.adjType) ) {
          ...
          setOutput(msg, node.outputType, node.output, '');
          msg.err = ('Adjustment measure type not valid, no adjustment made - check moment.js docs for valid measures (days, hours, etc)');
          node.send([null,msg]);
        }
      }
      ...
      if ( ! mDT.isValid() ) {
        ...
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('The input property was NOT a recognisable aaa date. Output will be a blank string');
        node.send([null,msg]);
      } else {
    ...
    function setOutput(msg, outputType, output, value) {
      try {
        switch ( outputType ) {
          ...
          default:
            setOutput(msg, node.outputType, node.output, '');
            msg.err = ('Unrecognised Output Type, ' + outputType + '. No output.');
            node.send([null,msg]);
        }
      } catch (err) {
        setOutput(msg, node.outputType, node.output, '');
        msg.err = ('Output property, ' + outputType + '.' + output + ', cannot be set. No output.', err);
        node.send([null,msg]);
      }
